...
import { AddService, updateService } from "@/Redux/ReduxFunctions";
...
export default function PostForm() {
  ...
  const onSubmit: SubmitHandler<FormData> = (
    ...
    formData.append('name', data.name || serviceData?.result?.name)
    formData.append('description', data.description || serviceData?.result?.description)
    formData.append('categoryId', data?.categoryId == "Loading" ? serviceData?.result?.categoryId : data?.categoryId)
    formData.append('serviceImage', data.file[0] || serviceData?.result?.image)

    if (serviceData?.result?.id) {
      const id = serviceData?.result?.id;
      console.log(id);
      const name = data?.name ? data.name : serviceData?.result?.name
    ...
    dispatch(AddService(formData)).then((res) => {
      console.log(res);
      if (res.payload) {
        refetch()
